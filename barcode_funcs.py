...
def plot_pept_barcode(
    ...
    if semi.shape[0] > 0 or tryptic.shape[0] > 0:
        ...
        up_pal_vals = sns.color_palette("Reds_d", color_levels)
        down_pal_vals = sns.color_palette("Blues_d", color_levels)
        #insig_pal_vals = sns.color_palette("Greys", color_levels)
        insig_pal_vals = sns.color_palette(['#FFFFFF', '#FFFFFF', '#FFFFFF'])
            ...
            for i in range(tryptic_fc_diff.shape[0]):
                if tryptic_fc_diff.iloc[i, 2] > 0:
                    ...
                    if tryptic_fc_diff.iloc[i, 3] < sig_thr:
                        ...
                        #print(tryptic_fc_diff.iloc[i, 1] - 1)
                        tryptic_bar_code[tryptic_fc_diff.iloc[i, 1] - 1] = (
                            insig_pal_vals[
                                np.ceil(
                                    min(
...
def plot_barcode(pal, ticklabel=None, barcode_name=None, ax=None, size=(10, 2)):
    ...
    ax.set_yticks([])
    #ax.set_yticks([0])
    #ax.set_yticklabels([barcode_name], fontweight='bold')
    ...
    ax.set_xticklabels(ticklabel, fontweight='bold')
